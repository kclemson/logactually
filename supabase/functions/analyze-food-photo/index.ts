...
import { buildPhotoAnalysisPrompt } from "../_shared/prompts.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Analyzing food photo, base64 length:', imageBase64.length);
    ...
    // Filter zero-nutrient items
    const validItems = mergedItems.filter(item =>
      item.calories > 0 || item.protein > 0 || item.carbs > 0 || item.fat > 0 ||
      (item.fiber || 0) > 0 || (item.sugar || 0) > 0 || (item.saturated_fat || 0) > 0 ||
      (item.sodium || 0) > 0 || (item.cholesterol || 0) > 0
    );
