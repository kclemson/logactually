...
BEGIN
  SELECT json_build_object(
    'total_users', (SELECT COUNT(*) FROM profiles WHERE id != exclude_user_id),
    'users_with_entries', (SELECT COUNT(DISTINCT user_id) FROM food_entries WHERE user_id != exclude_user_id),
    ...
    'daily_stats', (
      ...
      FROM (
        SELECT 
          ...
          (SELECT COUNT(DISTINCT user_id) FROM food_entries WHERE user_id != exclude_user_id AND eaten_date <= d.stat_date) as users_with_entries,
