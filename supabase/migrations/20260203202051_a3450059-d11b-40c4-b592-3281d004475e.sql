-- Update get_user_stats to use login_events table instead of profiles.login_count
...
BEGIN
  ...
  FROM (
    SELECT 
      ...
      p.user_number,
      p.is_read_only,
      -- Login counts from login_events table
      (SELECT COUNT(*) FROM login_events le WHERE le.user_id = p.id) as login_count,
      (SELECT COUNT(*) FROM login_events le WHERE le.user_id = p.id AND le.created_at > NOW() - INTERVAL '24 hours') as logins_today,
      ...
      -- Food details for today (for tooltip) - now with saved meal name
      (
      ...
      -- Weight details for today (for tooltip) - now with saved routine name
      (
        SELECT json_agg(json_build_object(
