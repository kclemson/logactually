...
BEGIN
  ...
  FROM (
    SELECT 
      ...
      (SELECT COUNT(*) FROM saved_charts sc WHERE sc.user_id = p.id) as saved_charts_count,
      (SELECT COUNT(*) FROM custom_log_types clt WHERE clt.user_id = p.id) as custom_log_types_count,
