...
export const FoodItemsTable = forwardRef<HTMLDivElement, FoodItemsTableProps>(
  (
    {
      items,
      editable = false,
      ...
      <div ref={ref} className="space-y-1">
        ...
        <TooltipProvider>
          {items.map((item, index) => {
            ...
            return (
              <div key={index} className={cn('grid gap-0.5 items-start', gridCols)}>
                {/* Name cell */}
                {editable ? (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div
                        ...
                        onBlur={(e) =>
                          onUpdateItem?.(index, 'name', e.currentTarget.textContent || '')
                        }
                        className={cn(
                ...
                {editable ? (
                  <>
                    <Input
                      ...
                      value={item.calories}
                      onChange={(e) =>
                        onUpdateItem?.(index, 'calories', Number(e.target.value))
                      }
                      className={cn(
                    ...
                    <Input
                      ...
                      value={item.protein}
                      onChange={(e) =>
                        onUpdateItem?.(index, 'protein', Number(e.target.value))
                      }
                      className={cn(
                        'h-7 !text-size-compact px-1 border-0 bg-transparent hover:bg-muted/50 focus:bg-muted/50',
                    ...
                    <Input
                      ...
                      value={item.carbs}
                      onChange={(e) =>
                        onUpdateItem?.(index, 'carbs', Number(e.target.value))
                      }
                      className={cn(
                        'h-7 !text-size-compact px-1 border-0 bg-transparent hover:bg-muted/50 focus:bg-muted/50',
                  ...
                  <>
                    <span className="text-size-compact px-1 py-1 text-muted-foreground">
                      {item.calories}
                    </span>
                    <span className="text-size-compact px-1 py-1 text-muted-foreground">
                      {item.protein}
        ...
        {showTotals && (
          <div
            className={cn(
              'grid gap-0.5 items-center pt-1 border-t !text-size-compact font-medium text-muted-foreground',
              gridCols
            )}
          >
...
FoodItemsTable.displayName = 'FoodItemsTable';
